...
parser.add_argument('--layer', default = '5', type = str, help = 'which folder is validation dataset?')
parser.add_argument('--test-folder', default = 'fold3', type = str, help = 'which folder is test dataset?')
...
def main():
    ...
    print('Which layer do we use for CAM? : ', args.layer)
    ...
    data_fold_list=[args.test_folder] # ['fold3']
    ...
    model = vgg11_bn(input_T1 = input_T1, input_DeepC = input_DeepC, \
            ...
            DeepC_isotropic = DeepC_isotropic, DeepC_isotropic_crop = DeepC_isotropic_crop, channel = args.channel, cnn_layer=Layer)
    model.to(device)
    ...
    if args.channel == 'DeepC':
        ...
        activation_path = './result/activation_Each_Layer/'+f'Layer{Layer}/' + folder
        ...
        for i, (data) in enumerate(tqdm(Test_dataloader)):
            ...
            gradients = model.get_activation_gradient() # weights... must be layer dependent

            pooled_gradients = torch.abs(torch.mean(gradients, dim = [0, 2, 3, 4]))

            activations = model.get_activations(input_data_T1, input_data_DeepC).detach() # feature maps... must be layer dependent
...
class VGG(nn.Module): 
    ...
    VGG model 
    '''
    def __init__(self, feature_extractor, input_T1, input_DeepC, double_vgg, double_vgg_share_param, DeepC_isotropic, DeepC_isotropic_crop, channel = None, cnn_layer=None):
    ...
    def forward(self, x1, x2):
        if self.double_vgg:
            ...
            if not self.double_vgg_share_param:
                ...
                if self.layer == '4':
                    model1=nn.Sequential(*list(self.feature_extractor_T1.children())[:27])
                    model2=nn.Sequential(*list(self.feature_extractor_T1.children())[27:])
                    x1=model1(x1)
                    x3=model2(x1)

                    model3=nn.Sequential(*list(self.feature_extractor_DeepC.children())[:27])
                    model4=nn.Sequential(*list(self.feature_extractor_DeepC.children())[27:])
                    x2=model3(x2)
                    x4=model4(x2)

                if self.layer == '3':
                    model1=nn.Sequential(*list(self.feature_extractor_T1.children())[:18])
                    model2=nn.Sequential(*list(self.feature_extractor_T1.children())[18:])
                    x1=model1(x1)
                    x3=model2(x1)

                    model3=nn.Sequential(*list(self.feature_extractor_DeepC.children())[:18])
                    model4=nn.Sequential(*list(self.feature_extractor_DeepC.children())[18:])
                    x2=model3(x2)
                    x4=model4(x2)

                if self.layer == '2':
                    model1=nn.Sequential(*list(self.feature_extractor_T1.children())[:9])
                    model2=nn.Sequential(*list(self.feature_extractor_T1.children())[9:])
                    x1=model1(x1)
                    x3=model2(x1)

                    model3=nn.Sequential(*list(self.feature_extractor_DeepC.children())[:9])
                    model4=nn.Sequential(*list(self.feature_extractor_DeepC.children())[9:])
                    x2=model3(x2)
                    x4=model4(x2) 

                if self.layer == '1':
                    model1=nn.Sequential(*list(self.feature_extractor_T1.children())[:4])
                    model2=nn.Sequential(*list(self.feature_extractor_T1.children())[4:])
                    x1=model1(x1)
                    x3=model2(x1)

                    model3=nn.Sequential(*list(self.feature_extractor_DeepC.children())[:4])
                    model4=nn.Sequential(*list(self.feature_extractor_DeepC.children())[4:])
                    x2=model3(x2)
                    x4=model4(x2)               	
            ...
            x = torch.cat((x3.view(x3.size(0), -1), x4.view(x4.size(0), -1)), dim = -1)
            ...
            print('==================T1================================')
            print('feature extraction T1:',self.feature_extractor)
            if self.layer == '5':
                x1 = self.feature_extractor(x1)
                x3 = x1

            if self.layer == '4':
                model1=nn.Sequential(*list(self.feature_extractor.children())[:27])
                model2=nn.Sequential(*list(self.feature_extractor.children())[27:])
                x1=model1(x1)
                x3=model2(x1)              

            if self.layer == '3':
                model1=nn.Sequential(*list(self.feature_extractor.children())[:18])
                model2=nn.Sequential(*list(self.feature_extractor.children())[18:])
                x1=model1(x1)
                x3=model2(x1)    
            
            if self.layer == '2':
                model1=nn.Sequential(*list(self.feature_extractor.children())[:9])
                model2=nn.Sequential(*list(self.feature_extractor.children())[9:])
                x1=model1(x1)
                x3=model2(x1)  

            if self.layer == '1':
                model1=nn.Sequential(*list(self.feature_extractor.children())[:4])
                model2=nn.Sequential(*list(self.feature_extractor.children())[4:])
                x1=model1(x1)
                x3=model2(x1)  	
            ...
            print('==================DeepC================================')
            print('feature extraction DeepC:',self.feature_extractor)
            if self.layer == '5':
                x2 = self.feature_extractor(x2)
                x4 = x2
            if self.layer == '4':
                model3=nn.Sequential(*list(self.feature_extractor.children())[:27])
                model4=nn.Sequential(*list(self.feature_extractor.children())[27:])
                x2=model3(x2)
                x4=model4(x2)   

            if self.layer == '3':
                model3=nn.Sequential(*list(self.feature_extractor.children())[:18])
                model4=nn.Sequential(*list(self.feature_extractor.children())[18:])
                x2=model3(x2)
                x4=model4(x2)   

            if self.layer == '2':
                model3=nn.Sequential(*list(self.feature_extractor.children())[:9])
                model4=nn.Sequential(*list(self.feature_extractor.children())[9:])
                x2=model3(x2)
                x4=model4(x2)   

            if self.layer == '1':
                model3=nn.Sequential(*list(self.feature_extractor.children())[:4])
                model4=nn.Sequential(*list(self.feature_extractor.children())[4:])
                x2=model3(x2)
                x4=model4(x2)
...
def vgg11_bn(input_T1, input_DeepC, double_vgg, double_vgg_share_param, DeepC_isotropic, DeepC_isotropic_crop, channel = None,  cnn_layer='5'):
    ...
    return VGG(make_layers(cfg['A'], input_T1, input_DeepC, double_vgg, double_vgg_share_param, batch_norm=True, se_block=True), \
        input_T1, input_DeepC, double_vgg, double_vgg_share_param, DeepC_isotropic, DeepC_isotropic_crop, channel, cnn_layer)
